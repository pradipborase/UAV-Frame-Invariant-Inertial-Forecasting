"""Publication figures. Vector PDF/SVG plus 300 dpi PNG. No model fitting."""
...
MODEL_ORDER = ["Persistence", "Best frozen Ridge", "TCN", "GRU", "Transformer"]
SHORT = {
    "Persistence": "Pers.",
    "Best frozen Ridge": "Ridge",
    "TCN": "TCN",
    "GRU": "GRU",
    "Transformer": "Trans.",
}
...
def figure3_horizon(horizon_rows: list[dict[str, Any]], path: Path) -> None:
...
def figure4_paired(paired_seq_rows: list[dict[str, Any]], path: Path) -> None:
    ...
    for ax, ctx in zip(axes.ravel(), CONTEXTS, strict=True):
        ...
        ax.set_xticks(x)
        ax.set_xticklabels([s.replace("_snapdragon", "") for s in seqs], rotation=40, ha="right", fontsize=6.5)
        ax.set_title(CTX_TITLE[ctx], fontsize=9)
        ax.set_ylabel(r"$\Delta$ RMSE (m/s$^2$); negative = lower than Ridge")
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
    handles, labels = axes[0, 0].get_legend_handles_labels()
...
def figure5_transfer_gap(gap_rows: list[dict[str, Any]], path: Path) -> None:
    fig, ax = plt.subplots(figsize=(7.6, 4.4))
    models = ["Best frozen Ridge", "TCN", "GRU", "Transformer"]
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(["Ridge", "TCN", "GRU", "Transformer"])
...
def figure6_high_qf(high_rows: list[dict[str, Any]], path: Path) -> None:
    fig, axes = plt.subplots(2, 2, figsize=(8.8, 6.6))
    x = np.arange(len(MODEL_ORDER), dtype=np.float64)
    width = 0.36
    for ax, ctx in zip(axes.ravel(), CONTEXTS, strict=True):
        all_m, hi_m = [], []
        for model in MODEL_ORDER:
            hit = [r for r in high_rows if r["context"] == ctx and r["model"] == model][0]
            all_m.append(float(hit["rmse_all"]))
            hi_m.append(float(hit["rmse_high_qf"]))
        ax.bar(x - width / 2, all_m, width=width, label="All targets" if ctx == "WITHIN_EUROC" else None, color="#bdbdbd", edgecolor="black", lw=0.4)
        ax.bar(x + width / 2, hi_m, width=width, label=r"High $q_f$ (seq. p95)" if ctx == "WITHIN_EUROC" else None, color="#636363", edgecolor="black", lw=0.4)
        ax.set_xticks(x)
        ax.set_xticklabels([SHORT[m] for m in MODEL_ORDER], fontsize=8)
        ax.set_title(CTX_TITLE[ctx], fontsize=9)
        ax.set_ylabel(r"Equal-sequence RMSE (m/s$^2$)")
